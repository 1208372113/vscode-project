...
void recvdata(int fd, int events, void *arg) {
  ...
  len = Read(fd, ev->buf,
             sizeof(ev->buf));  //读文件描述符, 数据存入myevent_s成员buf中
...
void initlistensocket() {
  ...
  //对事件结构体赋值
  /* void eventset(struct myevent_s *ev, int fd, void (*call_back)(int, int,
   * void *), void *arg);  */
  eventset(&g_events[MAX_EVENTS], g_lfd, acceptconn,
           &g_events[MAX_EVENTS]);  //仅仅是对g_events[MAX_EVENTS]进行设置
...
int main(int argc, char *argv[]) {
  ...
  struct epoll_event
      events[MAX_EVENTS + 1];  //保存已经满足就绪事件的文件描述符数组
  ...
  while (1) {
    ...
    for (i = 0; i < 1; i++, checkpos++) {
      ...
      long duration =
          now - g_events[checkpos].last_active;  //客户端不活跃的世间
